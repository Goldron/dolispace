...
<head>
    ...
    <title><?= esc(lang('Emails.confirmPasswordTitle')) ?></title>
    <style>
...
<body>
    <div class="container">
        ...
        <h1><?= esc(lang('Emails.confirmNewPasswordHeading')) ?></h1>

        <p><?= esc(lang('Emails.hello')) ?></p>
        <p><?= esc(lang('Emails.passwordChangeRequested')) ?></p>
        <p><?= lang('Emails.clickButtonToConfirmChange') ?></p>

        <a href="<?= esc($confirmUrl) ?>" class="btn"><?= esc(lang('Emails.confirmMyPasswordBtn')) ?></a>

        <p><?= lang('Emails.ifButtonDoesNotWork') ?></p>
        <p class="link"><?= esc($confirmUrl) ?></p>

        <p><?= esc(lang('Emails.ignoreIfNotRequestedPassword')) ?></p>
