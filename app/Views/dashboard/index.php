...
<div class="mb-8">
    <h1 class="text-xl font-semibold text-gray-900">
        <?= esc(lang('Dashboard.greeting', [(string) session()->get('user_name')])) ?> 👋
    </h1>
    <p class="mt-1 text-sm text-gray-500"><?= esc(lang('Dashboard.activityOverview')) ?></p>
</div>

<!-- Cartes de statistiques -->
<?php
$statCardCount = (cfg('facture_enabled', true) ? 1 : 0) + (cfg('propal_enabled', true) ? 1 : 0) + (cfg('commande_enabled', true) ? 1 : 0);
$statGridClass = match ($statCardCount) {
    0, 1    => 'grid-cols-1',
    2       => 'grid-cols-1 sm:grid-cols-2',
    default => 'grid-cols-1 sm:grid-cols-3',
};
?>
<div class="grid <?= $statGridClass ?> gap-4 mb-8">

    <!-- Factures -->
    <?php if (cfg('facture_enabled', true)): ?>
        <a href="<?= site_url('dashboard/invoices') ?>" class="bg-white rounded-xl border border-gray-200 p-5 hover:border-blue-200 hover:shadow-sm transition block">
            <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-gray-500"><?= esc(lang('Dashboard.invoicesCard')) ?></span>
            ...
            <p class="text-xs text-gray-400 mt-1"><?= esc(lang('Dashboard.recentSuffixF')) ?></p>
        </a>
    <?php endif ?>

    <!-- Devis -->
    <?php if (cfg('propal_enabled', true)): ?>
        <a href="<?= site_url('dashboard/proposals') ?>" class="bg-white rounded-xl border border-gray-200 p-5 hover:border-amber-200 hover:shadow-sm transition block">
            <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-gray-500"><?= esc(lang('Dashboard.proposalsCard')) ?></span>
            ...
            <p class="text-xs text-gray-400 mt-1"><?= esc(lang('Dashboard.recentSuffixM')) ?></p>
        </a>
    <?php endif ?>

    <!-- Commandes -->
    <?php if (cfg('commande_enabled', true)): ?>
        <a href="<?= site_url('dashboard/orders') ?>" class="bg-white rounded-xl border border-gray-200 p-5 hover:border-green-200 hover:shadow-sm transition block">
            <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-gray-500"><?= esc(lang('Dashboard.ordersCard')) ?></span>
...
<div class="bg-white rounded-xl border border-gray-200">
    ...
    <?php if (empty($recentLogs)): ?>
        <div class="px-5 py-10 text-center text-sm text-gray-400"><?= esc(lang('Dashboard.noActivityRecorded')) ?></div>
    <?php else: ?>
...
{
    return match ($action) {
        'login'            => lang('Dashboard.logLogin'),
        'logout'           => lang('Dashboard.logLogout'),
        'register'         => lang('Dashboard.logRegister'),
        'password_renewed' => lang('Dashboard.logPasswordRenewed'),
