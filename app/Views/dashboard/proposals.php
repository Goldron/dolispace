...
$statusLabels = [
    ...
    1 => ['label' => lang('Dashboard.statusInProgress'), 'class' => 'bg-amber-100 text-amber-700'],
    2 => ['label' => lang('Dashboard.statusSigned'),     'class' => 'bg-green-100 text-green-700'],
    3 => ['label' => lang('Dashboard.statusRefused'),    'class' => 'bg-red-100 text-red-700'],
];
?>

<div class="mb-8 flex items-center justify-between">
    <div>
        <h1 class="text-xl font-semibold text-gray-900"><?= esc(lang('Dashboard.proposalsTitle')) ?></h1>
        <p class="mt-1 text-sm text-gray-500"><?= count($proposals) ?> <?= esc(count($proposals) > 1 ? lang('Dashboard.proposalsFound') : lang('Dashboard.proposalFound')) ?></p>
    </div>
</div>
...
<?php if (empty($proposals)): ?>
    <div class="bg-white rounded-xl border border-gray-200 px-5 py-16 text-center">
        ...
        <p class="text-sm text-gray-400"><?= esc(lang('Dashboard.noProposalsAvailable')) ?></p>
    </div>
...
<?php else: ?>
    <div class="space-y-3">
        <?php foreach ($proposals as $i => $p): ?>
            ...
            $currency = doc_currency($p);
            $ttc      = fmt_money(doc_amount($p, 'total_ttc', $currency), $currency['symbol']);
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <div class="flex items-center justify-between text-xs text-gray-500">
                    <span><?= esc(lang('Dashboard.on')) ?> <?= $date ?> · <?= esc(lang('Dashboard.validUntil')) ?> <?= $expiry ?></span>
                    ...
                    <div id="<?= $collapseId ?>" class="hs-collapse hidden overflow-hidden transition-[height] duration-200">
                        <div class="mt-3 pt-3 border-t border-gray-100 space-y-2">
                            ...
                            <div class="flex items-center justify-between pt-2 mt-1 border-t border-gray-200 text-xs">
                                ...
                                <span class="font-bold text-gray-900"><?= fmt_money(doc_amount($p, 'total_ht', $currency), $currency['symbol']) ?></span>
                            </div>
                        </div>
                    </div>
                <?php endif ?>
