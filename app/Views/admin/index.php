...
/** @var array $userList */
/** @var array $uploadsList */
/** @var \CodeIgniter\Pager\Pager $logPager */
/** @var \CodeIgniter\Pager\Pager $uploadsPager */
/** @var string $search */
...
<div class="bg-white rounded-xl border border-gray-200 mb-8">
    ...
    <?php else: ?>
                    ...
                    <?php foreach ($uploadsList as $upload): ?>
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 text-gray-600">
                                <p><?= esc((string) ($upload['user_name'] ?? '—')) ?></p>
                                <p class="text-xs text-gray-400"><?= esc((string) ($upload['user_email'] ?? '')) ?></p>
                            </td>
                            <td class="px-5 py-3 text-gray-500 font-mono text-xs">
                                <?= esc((string) $upload['party_folder']) ?><?= ! empty($upload['ref_folder']) ? '/' . esc((string) $upload['ref_folder']) : '' ?>
                            </td>
                            <td class="px-5 py-3 text-gray-500 text-right"><?= $formatSize((int) $upload['size']) ?></td>
                            <td class="px-5 py-3 text-gray-500 text-right"><?= esc((string) $upload['created_at']) ?></td>
        ...
        <?php
            $currentPage = $uploadsPager->getCurrentPage('uploads');
            $totalPages  = $uploadsPager->getPageCount('uploads');
        ?>
        <?php if ($totalPages > 1): ?>
            <div class="px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-x-4">
                ...
                <div class="flex items-center gap-x-1">
                    ...
                    <?php if ($currentPage > 1): ?>
                        <a href="<?= $uploadsPager->getPageURI($currentPage - 1, 'uploads') ?>" class="inline-flex items-center justify-center size-8 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition">&#8592;</a>
                    <?php else: ?>
                    ...
                    <?php for ($p = 1; $p <= $totalPages; $p++):
                        ...
                        <a href="<?= $uploadsPager->getPageURI($p, 'uploads') ?>"
...
<?php if (cfg('uploads_page_enabled', true)): ?>
<!-- Modal de confirmation : vider la table des uploads -->
<div id="hs-clear-uploads-modal" class="hs-overlay hidden size-full fixed top-0 inset-s-0 z-80 overflow-x-hidden overflow-y-auto">
    <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-xl">
            <div class="flex justify-between items-center py-3 px-4 border-b border-gray-100">
