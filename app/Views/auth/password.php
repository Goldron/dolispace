...
<p class="text-sm text-gray-500 mb-6">
    <?= esc(lang('Auth.connectedAs')) ?> <span class="font-medium text-gray-700"><?= esc($email) ?></span>.
    <a href="<?= site_url('auth') ?>" class="text-blue-600 hover:underline ml-1"><?= esc(lang('Auth.edit')) ?></a>
</p>
...
<div class="relative my-5">
    ...
    <div class="relative flex justify-center text-xs"><span class="bg-white px-3 text-gray-400"><?= esc(lang('Auth.or')) ?></span></div>
</div>
...
<?= form_open('auth/request-otp') ?>
    <button type="submit" class="w-full py-2.5 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition cursor-pointer">
        ...
        <?= esc(lang('Auth.receiveCodeByEmail')) ?>
    </button>
<?= form_close() ?>
